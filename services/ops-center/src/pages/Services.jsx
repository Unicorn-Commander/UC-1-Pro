...
export default function Services() {
  const { services, controlService, wsConnected, fetchServices } = useSystem();
      ...
      {sortedServices.length > 0 ? (
        ...
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
            {services.length === 0 ? 'No services detected' : `No ${filterStatus} services found`}
          </h3>
...
function ServiceCard({ service, index, loading, onAction, onViewLogs, onViewDetails, serviceUrls }) {
  ...
  // Get service description and metadata
  const serviceInfo = getServiceInfo(service.name);
  const hasGPU = serviceInfo.gpu?.enabled;
    ...
    >
      ...
      <div className="mb-4">
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 leading-tight">
          {serviceInfo.description.slice(0, 80)}...
        </p>
      ...
      <div className="space-y-2 mb-4">
        ...
        {(service.port || serviceInfo.ports) && (
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center gap-1">
            ...
            <span className="font-medium text-gray-900 dark:text-white">
              {service.port || (serviceInfo.ports && serviceInfo.ports[0])}
            </span>
          </div>
        )}
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span>CPU: {service.cpu_percent ? `${service.cpu_percent.toFixed(1)}%` : '0%'}</span>
                      </div>
                      <div className="flex items-center gap-2">
